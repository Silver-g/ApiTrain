...
// писать отдельный интерфейс или лучше методы в один запихнуть и передвать их я хз как ндао ээээ памагите
...
// Прикинь походу указатель не нужен хотя мы прямо тут будем менять поля структуры внося туда токен класна
//
...
func (s *UserService) Login(loginData domain.LoginUserInternal) (string, error) {
	...
	if err != nil {
		...
		//мб тут еще есть внутренний сценраий ошибки
	}
...
// userData переменная которая тип данных указателья
// userData.PasswordHash и да хеш берешь тут умнчка
